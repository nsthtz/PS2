...
MPI_Datatype petri_selection_global_t;
...
void create_types(){
	...
	MPI_Type_vector(p_local_petri_x_dim - 2, p_local_petri_y_dim - 2, IMG_X, mpi_cell_t, &petri_selection_global_t);
...
/*	for (int i = 0; i < p_local_petri_x_dim*p_local_petri_y_dim; i++) {
		...
		local_petri_A[i].color = rank;
		local_petri_A[i].strength = i;	
...
void gather_petri(cell* newest_petri){
  //TODO: Gather the final petri for process rank 0
	...
	if (rank == 0) {
		global_petri = calloc(IMG_X*IMG_Y*IMG_X, sizeof(cell));	
	...
	for (int r = 1; r < size; r++) {
		...
		int i = (r%p_x_dims * (p_local_petri_x_dim - 2)) + (IMG_Y / p_y_dims) * ((r)/		p_y_dims)*IMG_X;
...
//	print_image(global_petri_image);

	} else {

		// buf, count, datatype, dest, tag, comm
		MPI_Send(&newest_petri[p_local_petri_x_dim + 1], 1, petri_selection_t, 0, 0, MPI_COMM_WORLD);
	}
